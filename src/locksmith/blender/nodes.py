...
def input_socket(node: Node, input_name: str) -> NodeSocket:
    socket: NodeSocket = node.inputs[input_name]
    return socket


def input_socket_at(node: Node, index: int) -> NodeSocket:
    socket: NodeSocket = node.inputs[index]
    return socket


def output_socket(node: Node, output_name: str) -> NodeSocket:
    socket: NodeSocket = node.outputs[output_name]
    return socket


def node_inputs(node: Node) -> List[NodeSocket]:
    sockets: List[NodeSocket] = list(node.inputs)
    return sockets


def node_outputs(node: Node) -> List[NodeSocket]:
    sockets: List[NodeSocket] = list(node.outputs)
    return sockets


def tree_nodes(node_tree: NodeTree) -> List[Node]:
    nodes: List[Node] = list(node_tree.nodes)
    return nodes
...
def set_color_ramp_stop(
    ...
    *,
    ...
    element: ColorRampElement = ramp.elements[index]
